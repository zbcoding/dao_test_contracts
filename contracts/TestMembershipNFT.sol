...
contract TestMembershipNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    modifier onlyVault() {
        require(msg.sender == vaultAddress, "TestMembershipNFT:: not the vault");
        _;
    }
    ...
    function withdrawTokenToVault(address _token) external onlyVault {
        require(_token != address(0), "TestMembershipNFT:: invalid token address");
    ...
    function setVaultAddress(address _newVaultAddress) external onlyOwner {
        require(
            ...
            "TestMembershipNFT:: invalid vault address"
        );
